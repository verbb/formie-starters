...
function readBrowserLocation(fallback?: BrowserLocationState): BrowserLocationState {
...
export default function StarterApp({ initialPathname = '/server-rendered/rest', initialSearch = '' }: StarterAppProps) {
  const initialLocation = useMemo<BrowserLocationState>(() => {
    return {
      pathname: initialPathname || '/server-rendered/rest',
      search: initialSearch || '',
    };
  }, [initialPathname, initialSearch]);
  const [location, setLocation] = useState<BrowserLocationState>(() => {
    return readBrowserLocation(initialLocation);
  });
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-3 shadow-[0_8px_24px_-18px_rgba(15,23,42,0.35)] md:px-8">
          ...
          >
            ...
            <div>
              <p className="text-lg font-semibold text-slate-900">Formie Next Starter</p>
            </div>
          </a>
        </div>
      </header>
