...
const NAV_ITEMS = [
  {
    mode: 'server-rendered' as const,
    transport: 'rest' as const,
    title: 'REST',
    to: { path: '/server-rendered/rest', query: { example: 'single-page', scenario: 'html-default-theme' } },
  },
  {
    mode: 'server-rendered' as const,
    ...
    title: 'GraphQL',
    to: {
      path: '/server-rendered/graphql',
      query: { example: 'single-page', demo: 'html-payload', scenario: 'html-default-theme' },
    },
  },
  {
    mode: 'client-rendered' as const,
    transport: 'rest' as const,
    title: 'REST',
    to: { path: '/client-rendered/rest', query: { example: 'single-page', scenario: 'component-rest-form' } },
  },
  {
    mode: 'client-rendered' as const,
    transport: 'graphql' as const,
    title: 'GraphQL',
    to: { path: '/client-rendered/graphql', query: { example: 'single-page', demo: 'component-payload' } },
  },
];
  ...
  <div>
      ...
      <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-6 py-3 shadow-[0_8px_24px_-18px_rgba(15,23,42,0.35)] md:px-8">
        ...
        >
          <div class="size-6 shrink-0">
            <svg viewBox="0 0 544.2 544.2" aria-hidden="true" class="size-full">
    ...
    <main class="mx-auto grid max-w-7xl gap-8 p-6 md:grid-cols-[14rem_minmax(0,1fr)] md:p-8">
      <aside class="md:sticky md:top-24 md:self-start">
        <nav class="space-y-6">
          <div v-for="sectionMode in (['server-rendered', 'client-rendered'] as const)" :key="sectionMode" class="space-y-2">
            <p class="px-3 text-sm font-semibold text-slate-900">
