...
export function buildDefaultGraphqlEndpoint(baseUrl: string): string {
  if (!baseUrl) {
    return '/api';
  }
  try {
...
export function createDemoExamples(handles: {
  singlePage: string;
  multiPage: string;
  advanced: string;
}): DemoExample[] {
...
export const REST_SCENARIOS: RestScenario[] = [
  ...
  },
  {
    ...
    summary: 'Load the public component contract and render it with a minimal starter-owned view.',
...
export function resolveExample(examples: DemoExample[], id: string | null): DemoExample {
