...
const location = ref<BrowserLocationState>(readBrowserLocation());

const activeLocation = computed(() => {
  return normalizeLocation(location.value.pathname) || {
...
watch(
  () => [location.value.pathname, location.value.search] as const,
  () => {
    if (normalizeLocation(location.value.pathname)) {
      return;
    }
    navigate(`/server-rendered/rest${location.value.search}`, { replace: true });
  },
  {
    immediate: true,
  },
...
<template>
  ...
  <main class="mx-auto grid max-w-7xl gap-8 p-6 md:grid-cols-[14rem_minmax(0,1fr)] md:p-8">
    ...
    <div class="space-y-6">
      <RestRoute
        v-if="activeLocation.transport === 'rest'"
        :key="`rest:${activeLocation.mode}`"
        :search="location.search"
        :presentation-mode="activeLocation.mode"
        @navigate="navigate"
      />
      <GraphqlRoute
        v-else
        :key="`graphql:${activeLocation.mode}`"
        :search="location.search"
        :presentation-mode="activeLocation.mode"
        @navigate="navigate"
      />
