/** Default when `VITE_FORMIE_BASE_URL` is unset (typical ddev HTTPS URL). Override in `.env`. */
const DEFAULT_FORMIE_BASE_URL = 'https://craft.ddev.site:8443';

function resolveFormieBaseUrl(): string {
  const fromEnv = import.meta.env.VITE_FORMIE_BASE_URL as string | undefined;
...
export const GRAPHQL_ENDPOINT = import.meta.env.VITE_FORMIE_GRAPHQL_ENDPOINT || buildDefaultGraphqlEndpoint(FORMIE_BASE_URL);
...
export const DEMO_EXAMPLES: DemoExample[] = [
  {
    ...
    description: 'Use this example as the baseline demo. It is the easiest way to compare how a normal production form looks and behaves under each transport.',
    handle: import.meta.env.VITE_FORMIE_SINGLE_PAGE_HANDLE || 'singlePage',
  ...
  },
  {
    ...
    description: 'Use this example to pressure-test workflow behavior such as page transitions, previous/next actions, and token-backed continuity.',
    handle: import.meta.env.VITE_FORMIE_MULTI_PAGE_HANDLE || 'multiPage',
  ...
  },
  {
    ...
    handle: import.meta.env.VITE_FORMIE_ADVANCED_HANDLE || 'advanced',
...
export const REST_SCENARIOS: RestScenario[] = [
  ...
  },
  {
    ...
    summary: 'Load the public component contract and render it with a minimal app-owned view.',
    description: 'This path uses `@verbb/formie-core` in vanilla JS (not `createFormieClient`) so REST and GraphQL component transports both work.',
  },
];
...
export const GRAPHQL_DEMOS: GraphqlDemo[] = [
  {
    ...
    description: 'This is the GraphQL-first equivalent of the REST HTML demo and the clearest path for teams standardizing reads on GraphQL.',
  },
  {
    id: 'component-payload',
    mode: 'client-rendered',
    title: 'Component payload query',
    summary: 'Fetch the dedicated frontend contract through GraphQL and map it into a minimal renderer.',
    description: 'Uses the same `formieClientForm` envelope as the React starter, rendered here with `createFrontendFormInstance` from `@verbb/formie-core`.',
  },
  {
    id: 'submit-mutation',
    mode: 'client-rendered',
    ...
    description: 'Static request body example aligned with the React starter. Live preview still mounts the client-rendered form (component-payload behavior).',
  },
];

export function resolveExample(id: string | null): DemoExample {
  return DEMO_EXAMPLES.find((example) => {
    return example.id === id;
  }) || DEMO_EXAMPLES[0];
}

export function resolveRestScenarioForMode(mode: RestScenario['mode'], id: string | null): RestScenario {
  return REST_SCENARIOS.find((scenario) => {
    return scenario.mode === mode && scenario.id === id;
...
export function resolveGraphqlDemoForMode(mode: GraphqlDemo['mode'], id: string | null): GraphqlDemo {
  return GRAPHQL_DEMOS.find((demo) => {
    return demo.mode === mode && demo.id === id;
  }) || GRAPHQL_DEMOS.find((demo) => {
